...
export async function saveSiteSettings(nextSettings) {
  ...
  // Ensure we only call revalidatePath with a pathname (no hash fragments)
  function stripHash(path = "") {
    return String(path).split("#")[0] || "/";
  }

  // Dynamically import `revalidatePath` to avoid bundling server-only API
  try {
    const { revalidatePath } = await import("next/cache");
    SITE_ROUTES.forEach((route) => {
      revalidatePath(stripHash(route.path));
    });
