...
function normalizeStudent(student, index) {
  ...
  return {
    ...
    image,
    imagePublicId: String(student.imagePublicId || "").trim(),
    title,
...
export async function getPlacedStudents() {
  try {
    ...
    // Always return actual DB data
    return normalizePlacedStudents(placement.students || []);
  } catch {
    return [];
  }
}
